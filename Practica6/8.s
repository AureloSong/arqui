...
loop:	sw     $v1, 0($s7)    	
        ...
        sb     $s1, 5($s7)			
        sd     $s4, 0($s6)			
        dadd   $s0, $s0, $s2                ; Calcula la nueva posición de la pelota
        dadd   $s1, $s1, $s3	            ; teniendo en cuenta la dir actual
        daddi  $t1, $zero, 48               ; Comprueba que la pelota no esté en la col 
        slt    $t0, $t1, $s0   		        ; de más a la derecha. Si es así, cambia la dir en X.
        dsll   $t0, $t0, 1
        dsub   $s2, $s2, $t0
